...
def predict(scores_array, 
	...
	for i in range(0, len(scores_array)):
		...
		name_genus_relu, score_genus_relu = predict_genus_relu(bin_array, 
												relu_genus_model, 
												genus_hosts
												)
		...
		name_genus_sm,\
		score_genus_sm,\
		entropy_genus_sm = predict_genus_softmax(
								bin_array,
								softmax_genus_model,
								genus_hosts
								)
		...
		name_species_relu,\
		score_species_relu = predict_species_relu(
								bin_array,
								relu_species_model,
								species_hosts
								)
		...
		name_species_sm,\
		score_species_sm = predict_species_softmax(
								bin_array,
								softmax_species_model,
								species_hosts
								)
		...
		# Verify if genus is the same
			...
			if re.search(name_genus_relu, final_decision) or \
				re.search(name_genus_sm, final_decision):
				pass
			else:
				# relu genus
				if score_genus_relu >= 0.9:
...
def main():
	...
	for faa in valid_faas.values():
		...
		count_hmms += 1
		print(
		"**Done with {} / {} HMMs\r".format(count_hmms, 
		len(valid_faas)), end = "")
	else:
		print("\n**Done with HMMscan!")
	...
	# genus relu
	genus_relu_h5 = models_dir / \
		Path("model_genus_total_fixed_relu_08mar_2020.h5")
	...
	# genus softmax
	genus_sm_h5 = models_dir / \
		Path("model_genus_total_fixed_softmax_01mar_2020.h5")
	...
	# species relu
	species_relu_h5 = models_dir / \
		Path("model_species_total_fixed_relu_08mar_2020.h5")
	...
	# species softmax
	species_sm_h5 = models_dir / \
		Path("model_species_total_fixed_softmax_01mar_2020.h5")
	...
	a = predict(mat_array, bin_names,
				model_genus_relu, model_genus_sm,
				model_species_relu, model_species_sm,
				genus_hosts, species_hosts)
